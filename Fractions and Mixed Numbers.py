...
if num < denom:
    ...
    if GCD == 1:
       ...
       print (f'The proper fraction can be reduced to: {int(num/GCD)} / {int(denom/GCD)}')
    ...
    mix = num // denom
    remain = (int(num/GCD))%(int(denom/GCD))   
    if remain != 0:
        print (f'The mixed number is {mix} and {remain} / {int(denom/GCD)}')
    else:
        print (f'The whole number is {int(num/GCD)}')