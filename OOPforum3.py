class shopitemQL: #create class for shopping items
    ...
    def __pricing(self): #private function to give the food items fixed prices
        if self.foodn.upper() == "DRY CURED IBERIAN HAM": #turning all the string input to upper case so the program can check whether or not the item has a fixed price
            self.price = 177.8
